...
\item \structure{Lists}: sequence of values (non-homogeneous), enclosed in brackets. Mutable
\item \structure{Sets}: no specific syntax \lstinline!set()! constructor is used.
...
class {\em classname\/}({\em optionalbaseclass\/}) :
   ...
   def __init__(self):
       ...
       self.y = 0
       self.cx += 1	# class member, classname.cx += 1 is similar
   def increment(self):
       self.x += 1
...
\defverbatim[colored]\codeiterator{
\begin{lstlisting}[escapeinside=\{\}]
for i in a:
   # loop body
...
\defverbatim[colored]\codefibgen{
\begin{lstlisting}[escapeinside=\{\}]
def fibonacci(n):
	a = 0
	b = 1
...
\defverbatim[colored]\codebstiter{
\begin{lstlisting}[basicstyle=\tiny\ttfamily,escapeinside=\{\}]
# ... added to BST
