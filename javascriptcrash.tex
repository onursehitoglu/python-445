...
\item \lstinline!document.querySelector! selects elements based on \structure{CSS selectors}.
...
\item for event type \structure{X}, attribute name is \structure{onX}
\begin{itemize}
...
\item call all asynchrous functions with \structure{await} prefix
...
\item Previous slide without \structure{await}
\begin{lstlisting}
main = () => {
	wait(1000)
		.then(() => {
...
\begin{frame}[fragile]
\begin{itemize}
\item \lstinline!getsum! \structure{async/await} version
\begin{lstlisting}
...
\item Alternatively \structure{XPath} can be used \lstinline!doc.evaluate('//flight[@no="LH1231"]', doc)!
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Shorter and easy to use \structure{CSS selectors} \lstinline!$(".id")!
...
\item \structure{CSRF} protects POST requests from other domains.
\item When there is a an AJAX request with custom headers or non standard (different content type)
	POST request, \structure{same origin} policy forbids making a request to another domain.
...
\If{origdomain=domain}
	\State \structure{Do actual request}
\ElsIf{$headers \not\subset$ standardheaders $or$\\ \hspace*{3em} ( $method=$POST $and$ contenttype is not standard)}
	\State make a GET or OPTIONS request to URL
	\State send \texttt{Origin: $origin$} header
	\If {Response is success with a matching \texttt{Access-Control-Allow-Origin} header}
		\State \structure{Do actual request}
	\Else
...
\begin{itemize}
	\item \structure{Long polling} Server does not respond immediately and keep connection open until
		data is available. Client repeatedly polls on connection break and/or new data.
	\item \structure{Server Sent Events} \lstinline!EventSource(URL)! request on client is responded
	...
	\item \structure{WebSocket} A bidirectional tunnel between server and client.
\end{itemize}
...
\item After this handshake, communication switches to a \structure{full duplex}, \structure{binary}
...
\item Websockets are not subject to \structure{same origin policy} so server check \texttt{Origin} header and authorize it  when establishing the connection.
\end{itemize}
\end{frame}
